...
class CameraManager:
    ...
    #     global frames_to_show
    #     if len(frames_to_show.keys()) > 1:
    #         frames_copy = dict(frames_to_show)  # Make a copy of the dictionary
    #     else:
    #         global latest_frames
    #         frames_copy = dict(latest_frames)
    #     for index, frame in frames_copy.items():
    ...
    def update_cv2_windows(self):
        ...
        for index, frame in frames_copy.items():
            ...
            if frame is not None:
                # Get current window size
                window_size = cv2.getWindowImageRect(window_name)
                resized_frame = cv2.resize(frame, (500, 800))
                cv2.imshow(window_name, resized_frame)
                cv2.imshow(window_name, frame)
                cv2.waitKey(1)
    ...
    def run_advanced(self):
        while True:
            ...
            global latest_frames
            frames_copy = dict(latest_frames)  # Make a copy of the dictionary
            image_list = []
            for index, frame in frames_copy.items():
            ...
            if len(image_list) > 1:
                ...
                results0, results1 = neural_network_process(self.detection_model, self.model, img0_undistorted, img1_undistorted)
