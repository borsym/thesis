...
video_poses = np.load("all_points_3d.npy", allow_pickle=True)
...
rot_matrix = o3d.geometry.get_rotation_matrix_from_xyz(
    (0, 0, np.radians(rot_degrees_per_frame)))
...
def translate_point_cloud(pcd, translation_vector):
    ...
    pcd.vertices = o3d.utility.Vector3dVector(
        np.asarray(pcd.vertices) + translation_vector)
...
def rotate_point_cloud(pcd, axis, angle):
    ...
    R = np.eye(3)
    if axis == 'x':
        R[1, 1], R[1, 2], R[2, 1], R[2, 2] = cos_angle, - \
            sin_angle, sin_angle, cos_angle
    elif axis == 'y':
        # angle_y += angle
        R[0, 0], R[0, 2], R[2, 0], R[2, 2] = cos_angle, sin_angle, - \
            sin_angle, cos_angle
    elif axis == 'z':
        # angle_z += angle
        R[0, 0], R[0, 1], R[1, 0], R[1, 1] = cos_angle, - \
            sin_angle, sin_angle, cos_angle
...
def keyboard_callback(vis, action, mods, pcd, base_trans_vector, id, scale_size, cylinder_list):
    ...
    if action == ord('W'):
        ...
        o3d.io.write_triangle_mesh(
            "D:/thesis/realtime_update/meshes/temp.ply", pcd)
    elif action == ord('O'):
        id += 1
        print(id)
...
vis.register_key_callback(87, lambda vis: keyboard_callback(vis, ord(
    'W'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'W'
vis.register_key_callback(65, lambda vis: keyboard_callback(vis, ord(
    'A'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'A'
vis.register_key_callback(68, lambda vis: keyboard_callback(vis, ord(
    'D'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'D'
vis.register_key_callback(83, lambda vis: keyboard_callback(vis, ord(
    'S'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'S'
vis.register_key_callback(82, lambda vis: keyboard_callback(vis, ord(
    'R'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'R'
vis.register_key_callback(84, lambda vis: keyboard_callback(vis, ord(
    'T'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'T'
vis.register_key_callback(79, lambda vis: keyboard_callback(vis, ord(
    'O'), 0, skeleton_cloud, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'O' frame +
vis.register_key_callback(76, lambda vis: keyboard_callback(vis, ord(
    'L'), 0, skeleton_cloud, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'L' frame -
vis.register_key_callback(77, lambda vis: keyboard_callback(vis, ord(
    'M'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'M' scale +
vis.register_key_callback(78, lambda vis: keyboard_callback(vis, ord(
    'N'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'N' scale -
vis.register_key_callback(66, lambda vis: keyboard_callback(vis, ord(
    'B'), 0, mesh, base_trans_vector, frame_id, scale_size, cylinder_list))  # Key 'B'

vis.register_key_callback(321, lambda vis: keyboard_callback2(
    vis, mesh, 321, 0, angle_x, angle_y, angle_z))  # Numpad 4
vis.register_key_callback(322, lambda vis: keyboard_callback2(
    vis, mesh, 322, 0, angle_x, angle_y, angle_z))  # Numpad 2
vis.register_key_callback(323, lambda vis: keyboard_callback2(
    vis, mesh, 323, 0, angle_x, angle_y, angle_z))  # Numpad 8
vis.register_key_callback(324, lambda vis: keyboard_callback2(
    vis, mesh, 324, 0, angle_x, angle_y, angle_z))  # Numpad 7
vis.register_key_callback(325, lambda vis: keyboard_callback2(
    vis, mesh, 325, 0, angle_x, angle_y, angle_z))  # Numpad 9
vis.register_key_callback(326, lambda vis: keyboard_callback2(
    vis, mesh, 326, 0, angle_x, angle_y, angle_z))  # Numpad 6
