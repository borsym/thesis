...
class CameraManager:
    def __init__(self, camera_int_params, camera_ext_params):
        ...
        mesh = o3d.io.read_triangle_mesh("meshes/dfki-2_full_mesh_fixed.ply")
        ...
        vid0_path = '/media/gables/Data/Camera Calibration/DFKI/Camera_Video/camera 0.mp4'
        vid1_path = '/media/gables/Data/Camera Calibration/DFKI/Camera_Video/camera 1.mp4'
        #
        processing = True
        ...
        out0 = cv2.VideoWriter(f'debug/DFKI_cam0.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 5, (2840, 2840))
        out1 = cv2.VideoWriter(f'debug/DFKI_cam1.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 5, (2840, 2840))
        ...
        while processing:
            ...
            if ret0 and ret1:
                ...
                vid0.release()
                vid1.release()
                np.save('demo_points3d_list_dfki.npy', self.points3D_list)
...
def calibrate_extrinsics(camera_int_params, chessboard_size = (9,6), square_size = 70):
    ...
    images_left = sorted(glob.glob('extrinsic_dfki/image_0_*.png'))  # Update path
    images_right = sorted(glob.glob('extrinsic_dfki/image_1_*.png'))  # Update path
...
def main():
    ...
    if os.path.exists(f'intrinsic/dfki_config.json'):
        print(f'loading intrinsics from config...')
        with open('intrinsic/dfki_config.json', 'r') as json_file:
            camera_int_params = json.load(json_file)
    else:
        print(f"The file 'intrinsic/dfki_config.json' does not exist. Closing app...")
        exit(0)
        ...
        with open('intrinsic/dfki_config.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        with open('intrinsic/dfki_config.json', 'r') as json_file:
            camera_int_params = json.load(json_file)
    if os.path.exists(f'extrinsic/dfki_config.json'):
        print(f'loading extrinsic from config...')
        with open('extrinsic/dfki_config.json', 'r') as json_file:
            camera_ext_params = json.load(json_file)
    else:
        print(f"The file 'extrinsic/dfki_config.json' does not exist. Closing app...")
        ...
        with open('extrinsic/dfki_config.json', 'w') as json_file:
            json.dump(data, json_file, indent=4)
        with open('extrinsic/dfki_config.json', 'r') as json_file:
            camera_ext_params = json.load(json_file)
    ...
    #
    # img0 = cv2.imread('intrinsic/dfki_door.jpg')
    # img1 = cv2.imread('intrinsic/dfki_window2.jpg')
    #
    # img0_undistorted = cv2.undistort(img0, mtx0_int, dist0_int, None, mtx0_int)
    # img1_undistorted = cv2.undistort(img1, mtx1_int, dist1_int, None, mtx1_int)
    #
    # cv2.imwrite('dfki_undistort_calib_cam0.png', img0_undistorted)
    # cv2.imwrite('dfki_undistort_calib_cam1.png', img1_undistorted)
